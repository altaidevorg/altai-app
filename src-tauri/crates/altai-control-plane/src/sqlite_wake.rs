...
impl WakeRepository for SqliteWakeRepository {
    ...
    fn checkout(&self, lease: WorkCheckoutLease) -> Result<(), WakeError> {
        let inserted = self.lock()?.execute(
            "INSERT INTO control_plane_work_checkout_leases (work_item_id, owner_agent_instance_id, attempt_id, expires_at) VALUES (?1, ?2, ?3, ?4) ON CONFLICT(work_item_id) DO NOTHING",
            params![lease.work_item_id.value, lease.owner_agent_instance_id.value, lease.attempt_id.value, lease.expires_at],
        ).map_err(Self::db)?;
        if inserted == 0 {
...
mod tests {
    ...
    fn wake_and_exclusive_lease_survive_reopen() {
        ...
        assert!(matches!(
            reopened.checkout(WorkCheckoutLease {
                work_item_id: work.clone(),

...
| 072 | Out-of-process plugin workers | in_progress | 071 | #815–#818 | Crash isolation, health, jobs, webhooks, scoped secrets and idempotency |
...
1. `CP-08-71` — Out-of-process plugin workers
   (Package 072 PR 5: the remaining capability pair over the same
   transport — webhook delivery and scoped-secrets hand-off, each gated
   on its declared capability, closing the 072 gate scope).

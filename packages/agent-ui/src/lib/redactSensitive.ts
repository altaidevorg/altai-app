/**
 * Pure redaction of common secret shapes in logs / transcripts (A6.149).
 * Keep patterns conservative — false positives are preferable to leaks.
 */
...
export function redactSensitive(text: string): string {
  ...
  for (const { kind, re } of PATTERNS) {
    // Fresh regex each replace so lastIndex state never leaks across calls.
    const pattern = new RegExp(re.source, re.flags);
    if (kind === "env-assign") {
      out = out.replace(
        pattern,
        (_m, name: string, q: string) => `${name}=${q}<REDACTED>${q}`,
      );
    } else {
